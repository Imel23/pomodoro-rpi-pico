...
/***************************************************************************************
** Function name:           ST7735_DrawLineThick
** Description:             Draw a line between 2 arbitrary points with customizable thickness
***************************************************************************************/

// Modified line drawing function with thickness
void ST7735_DrawLineThick(int16_t x0, int16_t y0, int16_t x1, int16_t y1, uint16_t color, uint16_t thickness)
{
  ...
  for (; x0 <= x1; x0++)
  {
    if (steep)
    {
      // Draw a rectangle centered at (y0, x0)
      ST7735_FillRectangle(y0 - half_thickness, x0 - half_thickness, thickness, thickness, color);
    }
    else
    {
      // Draw a rectangle centered at (x0, y0)
      ST7735_FillRectangle(x0 - half_thickness, y0 - half_thickness, thickness, thickness, color);
    }
    err -= dy;
    if (err < 0)
    {
